...
!(function (e, t) {
	...
	class b {
			...
			if (t) {
				...
				for (; e; ) {
					...
					this.options.gfm && ((k = /^\[[ xX]\] /.exec(o)), k && ((u = "[ ] " !== k[0]), (o = o.replace(/^\[[ xX]\] +/, "")))),
						r.items.push({ type: "list_item", raw: s, task: !!k, checked: u, loose: !1, text: o, tokens: [] }),
						(r.raw += s);
				}
		...
		link(e) {
			...
			if (t) {
				...
				return (
					...
					x(
						t,
						{ href: n ? n.replace(this.rules.inline.anyPunctuation, "$1") : n, title: s ? s.replace(this.rules.inline.anyPunctuation, "$1") : s },
						t[0],
						this.lexer
					)
				);
		...
		autolink(e) {
			...
			if (t) {
				let e, n;
				return (
					"@" === t[2] ? ((e = c(t[1])), (n = "mailto:" + e)) : ((e = c(t[1])), (n = e)),
	...
	const w = /^ {0,3}((?:-[\t ]*){3,}|(?:_[ \t]*){3,}|(?:\*[ \t]*){3,})(?:\n+|$)/,
		...
		v = /^( {2,}|\\)\n(?!\s*$)/,
		Q = "\\p{P}\\p{S}",
		B = u(/^((?![*_])[\spunctuation])/, "u")
			.replace(/punctuation/g, Q)
			.getRegex(),
		C = u(/^(?:\*+(?:((?!\*)[punct])|[^\s*]))|^_+(?:((?!_)[punct])|([^\s_]))/, "u")
			.replace(/punct/g, Q)
			.getRegex(),
		M = u(
		...
		V = {
			...
			url: u(/^((?:ftp|https?):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/, "i")
		...
		Y = { normal: E, gfm: Z, pedantic: L },
		ee = { normal: J, gfm: V, breaks: W, pedantic: K };
	class te {
		tokens;
		options;
		state;
		...
		constructor(t) {
			...
			const n = { block: Y.normal, inline: ee.normal };
			this.options.pedantic
				? ((n.block = Y.pedantic), (n.inline = ee.pedantic))
				: this.options.gfm && ((n.block = Y.gfm), this.options.breaks ? (n.inline = ee.breaks) : (n.inline = ee.gfm)),
				(this.tokenizer.rules = n);
		}
		static get rules() {
			return { block: Y, inline: ee };
		}
		static lex(e, t) {
			...
			for (
				...
				if (
					!(
						...
						this.options.extensions.block.some(n => !!(s = n.call({ lexer: this }, e, t)) && ((e = e.substring(s.raw.length)), t.push(s), !0))
					)
				)
					if ((s = this.tokenizer.space(e)))
						(e = e.substring(s.raw.length)), 1 === s.raw.length && t.length > 0 ? (t[t.length - 1].raw += "\n") : t.push(s);
				...
				if (
					...
					if ((n = this.tokenizer.escape(e))) (e = e.substring(n.raw.length)), t.push(n);
						...
						(e = e.substring(n.raw.length)),
							(s = t[t.length - 1]),
							s && "text" === n.type && "text" === s.type ? ((s.raw += n.raw), (s.text += n.text)) : t.push(n);
			...
			for (let s = 0; s < e.length; s++) {
				...
				switch (i.type) {
					...
					case "text": {
						...
						n += t ? this.renderer.paragraph({ type: "paragraph", raw: l, text: l, tokens: [{ type: "text", raw: l, text: l }] }) : l;
						continue;
					}
					default: {
	...
	class le {
		...
		#t(e, t, n) {
			switch (t) {
				case "heading":
					return function (s) {
						return s.type && s.type === t
							? e.call(
									this,
									n.parser.parseInline(s.tokens),
									s.depth,
									(function (e) {
				...
				case "table":
					return function (n) {
						...
						for (let e = 0; e < n.header.length; e++)
							r += this.tablecell({ text: n.header[e].text, tokens: n.header[e].tokens, header: !0, align: n.align[e] });
						s += this.tablerow({ text: r });
						let i = "";
						for (let e = 0; e < n.rows.length; e++) {
							const t = n.rows[e];
							r = "";
							for (let e = 0; e < t.length; e++) r += this.tablecell({ text: t[e].text, tokens: t[e].tokens, header: !1, align: n.align[e] });
				...
				case "list":
					return function (n) {
						...
						for (let e = 0; e < n.items.length; e++) {
							...
							let o = "";
							if (t.task) {
								const e = this.checkbox({ checked: !!s });
								i
							...
							(o += this.parser.parse(t.tokens, i)),
								(l += this.listitem({ type: "list_item", raw: o, text: o, task: r, checked: !!s, loose: i, tokens: t.tokens }));
						}
	...
	(ae.options = ae.setOptions =
		function (e) {
			return oe.setOptions(e), (ae.defaults = oe.defaults), n(ae.defaults), ae;
		}),
		(ae.getDefaults = t),
		(ae.defaults = e.defaults),
		(ae.use = function (...e) {
			return oe.use(...e), (ae.defaults = oe.defaults), n(ae.defaults), ae;
		}),
